...
Eloquent Model Generator is a tool based on [Code Generator](https://github.com/d-usmonov/code-generator) for generating Eloquent models.
...
```
php artisan modelyarat:generate:model User --output-path=/full/path/to/output/directory --namespace=Some\\Other\\NSpace
```
...
```
php artisan modelyarat:generate:model User --base-class-name=Some\\Other\\Base\\Model
```

### backup
Save existing model before generating a new one
```
php artisan modelyarat:generate:model User --backup
```
