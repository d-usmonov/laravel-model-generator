...
## Version 2.0.0
Version 2.0.0 has been released. Checkout [this discussion](https://github.com/d-usmonov/laravel-model-generator/discussions/89) for more details and upgrade instructions.

## Installation
Step 1. Add Laravel Eloquent Model Generator to your project:
```
composer require d-usmonov/laravel-model-generator --dev
```
Step 2. Register `GeneratorServiceProvider`:
```php
'providers' => [
    // ...
    DUsmonov\LaravelModelGenerator\Provider\GeneratorServiceProvider::class,
];
```
...
Use
```
php artisan modelyarat:generate:model User
```
...
```
php artisan modelyarat:generate:model User --table-name=user
```
...
```
php artisan modelyarat:generate:model User --output-path=/full/path/to/output/directory --namespace=Your\\Custom\\Models\\Place
```
...
```
php artisan modelyarat:generate:model User --base-class-name=Custom\\Base\\Model
```
...
```
php artisan modelyarat:generate:model User --no-backup
```
...
Instead of specifying options each time when executing the command you can create a config file named `laravel_model_generator.php` at project's `config` directory with your own default values:
```php
<?php
...
```
it means that you must register your type `<TYPE>` at your `config/laravel_model_generator.php`:
...
Command `modelyarat:generate:models` will generate models for all tables in the database. It accepts all options available for `modelyarat:generate:model` along with `skip-table` option.

### skip-table
Specify one or multiple table names to skip:
```php
php artisan modelyarat:generate:models --skip-table=users --skip-table=roles
```
Note that table names must be specified without prefix if you have one configured.

## Customization
You can hook into the process of model generation by adding your own instances of `DUsmonov\LaravelModelGenerator\Processor\ProcessorInterface` and tagging it with `GeneratorServiceProvider::PROCESSOR_TAG`.
